...
module.exports = {
  ...
  run: (client, interaction) => {
    ...
    const embed = new EmbedBuilder()
      ...
      .setFooter({ text: `Total Queued • ${player.queue.length} tracks`, iconURL: client.user.displayAvatarURL({ dynamic: true }) })
      .setTimestamp();
      ...
      embed.addFields([
        {
          name: 'Up Next',
          value: queue
            .map((track, index) => `**${index + 1}.** [${track.info.title}](${track.info.uri}) • \`${formatDuration(track.info.length)}\``)
            .join('\n'),
        },
      ]);
