...
module.exports = {
  ...
  run : (client, interaction)=> {
    ...
    const embed = new EmbedBuilder()
      .setAuthor({ name: player.isPlaying ? `Now Playing` : `Song Paused`, iconURL: 'https://cdn.discordapp.com/attachments/1014342568554811443/1025740239236517908/music-disc.gif' })
      .setThumbnail(player.currentTrack.info.image)
      .setDescription(`**[${player.currentTrack.info.title}](${player.currentTrack.info.uri})**`)
      .addFields({ name: `Author:`, value: `${player.currentTrack.info.author}`, inline: true })
      .addFields({ name: `Requested By:`, value: `${player.currentTrack.info.requester}`, inline: true })
      .addFields({ name: `Source:`, value: `${sources}`, inline: true })
      .addFields({ name: `Duration:`, value: `${trackDuration}`, inline: true })
      .addFields({ name: `Volume:`, value: `${player.filters.volume * 100}%`, inline: true })
      .addFields({ name: `Queue Left:`, value: `${player.queue.length}`, inline: true })
      .addFields({ name: `Song Progress: \`[${currentPosition}]\``, value: `\`\`\`${Emoji} ${'─'.repeat(Part) + '🔵' + '─'.repeat(30 - Part)}\`\`\``, inline: false })
